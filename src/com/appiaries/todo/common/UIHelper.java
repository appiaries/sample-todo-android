...
public class UIHelper {
	 ...
	 * @return
	 */
	public static AlertDialog createNotifyDialog(Context context,
			String content, String imageType) {

		// Instantiate an AlertDialog.Builder with its constructor
		AlertDialog.Builder builder = new AlertDialog.Builder(context);

		/*
		 * if (imageType.equals("update")) { TextView customMessage = new
		 * TextView(context);
		 * 
		 * customMessage.setPadding(10, 40, 10, 40);
		 * customMessage.setText(content);
		 ...
		 * builder.setView(customMessage);
		 * 
		 * builder.setCancelable(false); } else { LayoutInflater mInflater =
		 * (LayoutInflater) context
		 * .getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		 * 
		 * View dialogView = mInflater.inflate(R.layout.dialog_image, null,
		 * false);
		 * 
		 * TextView dialogMessage = (TextView) dialogView
		 * .findViewById(R.id.dialogMessage);
		 * 
		 * ImageView dialogImage = (ImageView) dialogView
		 * .findViewById(R.id.dialogImage); dialogMessage.setText(content);
		 * 
		 * if (imageType.equals("success")) {
		 * dialogImage.setImageDrawable(dialogView.getResources()
		 * .getDrawable(R.drawable.success_black)); } else {
		 * dialogImage.setImageDrawable(dialogView.getResources()
		 * .getDrawable(R.drawable.error_black)); } builder.setView(dialogView);
		 * }
		 */
