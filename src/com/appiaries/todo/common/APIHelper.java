...
public class APIHelper {
	...
	public static String getUserToken(Context ctx) {
		SharedPreferences pref = ctx.getSharedPreferences(Constants.PREFS_NAME,
				0);
		String accessToken = pref.getString(Constants.USER_TOKEN_KEY, "");
	...
	public static String getOAuthUrlWithScope(String scope) {
		...
		urlHelper.addQuery("client_id", Constants.APIS_CLIENT_ID);
	...
	public static String getProfileAPIWithCompletion(Context ctx) {
		...
		return GET(url, APIHelper.getStoreToken(ctx),
				APIHelper.getUserToken(ctx));
	}
	
	public static boolean isAlreadyRegistered(Context ctx) {
		SharedPreferences pref = ctx.getSharedPreferences(Constants.PREFS_NAME,
				0);
		String accessToken = pref.getString(Constants.USER_TOKEN_KEY, null);
	...
	public static boolean iSNSAccount(Context ctx) {
		SharedPreferences pref = ctx.getSharedPreferences(Constants.PREFS_NAME,
				0);
		boolean isSNSAccount = pref.getBoolean(Constants.IS_SNS_ACCOUNT, false);
		
		return isSNSAccount;
	}

	public static void setIsSNSAccount(Context ctx, boolean isSNSAccount) {
		SharedPreferences pref = ctx.getSharedPreferences(Constants.PREFS_NAME,
				0);
		Editor editor = pref.edit();

		editor.putBoolean(Constants.IS_SNS_ACCOUNT, isSNSAccount);
		editor.commit();
	}
